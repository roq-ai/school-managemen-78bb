...
type ColumnType = ColumnDef<RenamedclassInterface, unknown>;

interface RenamedclassListPageProps {
...
export function RenamedclassListPage(props: RenamedclassListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRenamedclasses({
        relations: ['academic_year', 'user', 'assignment.count', 'attendance.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'section.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RenamedclassInterface>>(
    () => `/renamedclasses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RenamedclassInterface) => {
    if (hasAccess('renamedclass', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/renamedclasses/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/renamedclasses/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="renamedclass" />
        </Flex>
