...
import { AttendanceListPage as ShowCaseList } from 'pages/attendances';
...
function DashboardPage() {
  ...
  const renamedclassStat = useSWR<PaginatedInterface<RenamedclassInterface>>(
    () => `/renamedclasses`,
    () => getRenamedclasses({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Renamedclasses',
      result: renamedclassStat?.data?.totalCount,
      isLoading: renamedclassStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`attendances`} passHref legacyBehavior>
                  <Button
